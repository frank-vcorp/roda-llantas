# SPEC-SALES-FIX: Transacción Atómica con RPC

## Problema Detectado
La implementación inicial realizaba operaciones secuenciales (Check -> Insert -> Update) desde el cliente/servidor, expuesta a condiciones de carrera y corrupción de datos si un paso intermedio falla.

## Solución: Procedimiento Almacenado (RPC)
Se debe crear una función PostgreSQL `confirm_sale` que encapsule toda la lógica en una transacción ACID.
...
## Acción Requerida
1. Crear migración con este SQL.
2. Actualizar `sales.ts` para llamar `supabase.rpc('confirm_sale', { p_quotation_id: id })`.
